...
export default (store) => (next) => (action) => {
  ...
  switch (action.type) {
    case ADD_TASK_SUBMIT:
      ...
      axios.post(`http://3.90.247.93:8080/api/dashboard/${userId}/task`, {
      ...
      break;
    case DELETE_TASK:
      axios.delete(`http://3.90.247.93:8080/api/dashboard/${userId}/task/${action.taskId}`, {
        title: store.getState().todolist.labelNewTask,
      }, { withCredentials: true })
        .then((res) => {
          // console.log(res.data);
          axios.get(`http://3.90.247.93:8080/api/dashboard/${userId}/task`)
            .then((res2) => {
